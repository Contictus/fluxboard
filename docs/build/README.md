...
- **Phase:** 7 — Frontend (IN PROGRESS: sections 0–6 done; next §7 org settings)
- **Next check:** PHASE-7 §7 (`/settings` general → `/settings/members` + invite → `/settings/labels` → `/settings/api-keys` + `/settings/audit-log` + `/settings/danger`). Org member list (`GET /orgs/{id}/members`, wrapped as `listOrgMembers`) already exists from §6 — reuse it; ChangeMemberRole/invitation endpoints in orgs.go need contract-reading before wiring.
- **Last verified commit:** Phase 7 §6 task detail + search/trash/my-tasks (this commit); §5 `7ed8bee`, §4 `9342df3`, §3 `a0a924e`, §0 `e64737a`, §1 `aeaafbd`, §2 `b9b7f55`, §B backend `c6b45a4`, tracker `c2c79ca`
- **Phase 7 progress:** §0 scaffold (Next 14 App Router, TS strict, pnpm, hand-written typed fetch client — OpenAPI too partial for codegen), §1 public/marketing, §2 auth flows, §B backend gap-fill (`/me`; ADR-015), §3 account, §4 org shell (`/app/{orgSlug}` shell + home; ADR-016), §5 projects + kanban (dnd-kit board, optimistic LexoRank move + 409 rollback, filters + bulk, list view, settings; key→UUID client resolve + rank port, ADR-017), §6 task detail + org task views (`/projects/{key}/tasks/{number}` full-page detail — inline edit all fields, activity, subtasks, comments w/ @mention, attachments presigned PUT/GET; number→id via board projection; `/search` faceted, `/my-tasks` grouped, `/trash` per-project fan-out restore-only; member names via `listOrgMembers`; ADR-018). All web gates green (`pnpm typecheck && lint && build`, **37 routes**); backend `go build/vet/test` green (160/49). **Not yet run:** dockerized e2e for the new UI (build-verified only). Remaining: §7–§11 (org settings, billing UI, SSE client, admin, analytics, verify+commit).
