...
- **Phase:** 7 — Frontend (IN PROGRESS: sections 0–4 done; next §5 projects + kanban)
- **Next check:** PHASE-7 §5 (`/projects` list/grid → `/projects/new` → `/projects/{key}` Kanban board dnd-kit)
- **Last verified commit:** Phase 7 §4 org shell (this commit); §3 `a0a924e`, §0 `e64737a`, §1 `aeaafbd`, §2 `b9b7f55`, §B backend `c6b45a4`, tracker `c2c79ca`
- **Phase 7 progress:** §0 scaffold (Next 14 App Router, TS strict, pnpm, hand-written typed fetch client — OpenAPI too partial for codegen), §1 public/marketing, §2 auth flows (login/2fa/register/verify/reset/oauth/invite/logout), §B backend gap-fill (`/me` profile+avatar+delete; ADR-015), §3 account (profile/security/sessions/notifications/danger), §4 org shell (`/app/{orgSlug}` sidebar+topbar+role-aware nav, org switcher, org home [assigned-to-me / notifications-as-activity / recent projects], past_due banner + soft-delete restore screen; org context = `GET /orgs` role + `GET /orgs/{id}` deleted_at, ADR-016). All web gates green (`pnpm typecheck && lint && build`, 27 routes); backend `go build/vet/test` green (160/49). **Not yet run:** dockerized e2e for the new UI (build-verified only). Remaining: §5–§11 (projects/kanban, task detail, org settings, billing UI, SSE client, admin, analytics, verify+commit).
