...
- **Phase:** 7 — Frontend (IN PROGRESS: sections 0–5 done; next §6 task detail)
- **Next check:** PHASE-7 §6 (`/projects/{key}/tasks/{number}` intercepted modal/full-page task detail → inline edit + activity → subtasks/comments → attachments → search/my-tasks/trash)
- **Last verified commit:** Phase 7 §5 projects + kanban (this commit); §4 `9342df3`, §3 `a0a924e`, §0 `e64737a`, §1 `aeaafbd`, §2 `b9b7f55`, §B backend `c6b45a4`, tracker `c2c79ca`
- **Phase 7 progress:** §0 scaffold (Next 14 App Router, TS strict, pnpm, hand-written typed fetch client — OpenAPI too partial for codegen), §1 public/marketing, §2 auth flows, §B backend gap-fill (`/me`; ADR-015), §3 account, §4 org shell (`/app/{orgSlug}` shell + home; ADR-016), §5 projects + kanban (`/projects` list/create, `/projects/{key}` dnd-kit board with optimistic LexoRank move + 409 rollback, filters + bulk `/tasks/bulk`, list view, project settings [general/columns/archive]; key→UUID resolved client-side + client rank port `lib/board/rank.ts`, ADR-017; added dep `@dnd-kit/{core,sortable,utilities}`). All web gates green (`pnpm typecheck && lint && build`, **33 routes**); backend `go build/vet/test` green (160/49). **Not yet run:** dockerized e2e for the new UI (build-verified only). Remaining: §6–§11 (task detail, org settings, billing UI, SSE client, admin, analytics, verify+commit).
