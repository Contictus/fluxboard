...
func TestSignVerifyRoundTrip(t *testing.T) {
	...
	tok, err := s.Sign("user-123", "sess-abc", true, time.Now(), 15*time.Minute)
	if err != nil {
		t.Fatal(err)
	}
	...
	if !claims.Ver {
		t.Fatalf("expected ver=true claim")
	}
}
...
func TestVerifyRejectsExpired(t *testing.T) {
	s := newTestSigner(t)
	v := VerifierFromSigners(s)
	tok, _ := s.Sign("u", "sid", true, time.Now().Add(-time.Hour), 15*time.Minute)
...
func TestVerifyRejectsUnknownKID(t *testing.T) {
	...
	tok, _ := signer.Sign("u", "sid", true, time.Now(), 15*time.Minute)
	if _, err := v.Verify(tok); err == nil {
